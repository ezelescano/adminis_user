...
@section('content')
    <div class="conteiner">
        ...
        <div>
            <a class="ml-2 text-xs hover:underline text-blue-300" href="{{ route('users.edit', $user) }}">Editar Usuario</a>
        </div>

        <form class=" ml-2 text-xs hover:underline text-red-300" action="{{ route('users.destroy', $user) }}" method="POST">
            @csrf
